...
def _boundary_cols(df: pd.DataFrame):
    return [c for c in df.columns if c.startswith("boundary.r_cos") or c.startswith("boundary.z_sin")]
...
def train_simple_mlp(cache_dir: Path, output_dir: Path):
    ...
    target_col = next((c for c in df.columns if c.startswith("metrics.") and df[c].dtype != object), None)
